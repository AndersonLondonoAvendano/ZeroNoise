...
class ReachabilityResult(BaseModel):
    ...
    def auto_justification(self) -> str:
        if not self.is_reachable:
            return (
                f"[ZeroNoise Stage 2] El paquete '{self.package}' está instalado como "
                f"dependencia pero nunca es importado o requerido por ningún archivo "
                f"fuente de la aplicación ({self.files_scanned} archivos escaneados). "
                "Marcado como NO_AFECTADO — no alcanzable desde el código de la aplicación."
            )
        files = ", ".join(u.file for u in self.usages[:3])
        extra = f" (+{len(self.usages) - 3} más)" if len(self.usages) > 3 else ""
        return (
            f"[ZeroNoise Stage 2] El paquete '{self.package}' es importado en "
            f"{len(self.usages)} archivo(s): {files}{extra}. "
            "Requiere análisis contextual de Stage 3."
        )
